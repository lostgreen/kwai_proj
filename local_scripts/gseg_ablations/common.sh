#!/usr/bin/env bash
# =============================================================
# common.sh — Grounding+Segmentation (Segment-as-CoT) 消融实验共用超参数
#
# 与 hier_seg / AOT / TG 消融实验对齐，便于跨任务对比。
#
...
# ---- 通用实验标签 ----
PROJECT_NAME="${PROJECT_NAME:-EasyR1-gseg-ablation}"

# ---- 模型 ----
MODEL_PATH="${MODEL_PATH:-/home/xuboshen/models/Qwen3-VL-4B-Instruct}"

# ---- 数据路径 ----
GSEG_DATA_ROOT="${GSEG_DATA_ROOT:-/m2v_intern/xuboshen/zgw/data/VideoProxyMixed/grounding_seg}"
TRAIN_FILE="${TRAIN_FILE:-${GSEG_DATA_ROOT}/train_data/train.jsonl}"
TEST_FILE="${TEST_FILE:-${GSEG_DATA_ROOT}/train_data/val.jsonl}"
...
# ---- Reward: reuse hier_seg reward (F1-IoU fallback for <events> format) ----
...
CHECKPOINT_ROOT="${CHECKPOINT_ROOT:-/m2v_intern/xuboshen/zgw/RL-Models/VideoProxyMixed/grounding_seg/ablations}"
