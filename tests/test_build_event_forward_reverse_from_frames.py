...
from video_proxy.data.pipelines.proxy_construction.event_progression.build_forward_reverse_frame_list_data import (
...
def test_cli_writes_records_for_tiny_fake_cache(tmp_path: Path):
    ...
    subprocess.run(
        [
            sys.executable,
            "video_proxy/data/pipelines/proxy_construction/event_progression/build_forward_reverse_frame_list_data.py",
            "--event-manifest",
            str(manifest),
            "--frames-root",
            str(tmp_path / "frames"),
