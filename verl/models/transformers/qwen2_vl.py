...
def _get_input_embeds(
    ...
    if pixel_values is not None:
        ...
        # [NEW] When image is missing: use a tiny dummy input to "activate" the image branch (hook into the graph without changing values)
        config = model.config.vision_config
        patch_dim = config.in_channels * config.temporal_patch_size * (config.patch_size ** 2)
        _dummy_pixels_img = torch.zeros(
            (16, patch_dim),
            dtype=inputs_embeds.dtype,
            device=inputs_embeds.device
        ).type(model.visual.dtype)
        _dummy_grid_img = torch.tensor([[1, 4, 4]], dtype=torch.long, device=inputs_embeds.device)
        _dummy_img_embeds = model.visual(_dummy_pixels_img, grid_thw=_dummy_grid_img)
        inputs_embeds = inputs_embeds + 0.0 * _dummy_img_embeds.mean()  # Values unchanged, but graph/gradients connected
    ...
    if pixel_values_videos is not None:
        ...
        # [NEW] When video is missing: use a tiny dummy input to "activate" the video branch (hook into the graph without changing values)
        config = model.config.vision_config
        patch_dim = config.in_channels * config.temporal_patch_size * (config.patch_size ** 2)
        _dummy_pixels_vid = torch.zeros(
            (16, patch_dim),
            dtype=inputs_embeds.dtype,
            device=inputs_embeds.device
        ).type(model.visual.dtype)
        _dummy_grid_vid = torch.tensor([[1, 4, 4]], dtype=torch.long, device=inputs_embeds.device)
        _dummy_vid_embeds = model.visual(_dummy_pixels_vid, grid_thw=_dummy_grid_vid)
        inputs_embeds = inputs_embeds + 0.0 * _dummy_vid_embeds.mean()  # Values unchanged, but graph/gradients connected
