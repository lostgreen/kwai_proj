...
def _maybe_log_model_video_inputs(
    model: "Qwen3VLModel",
    ...
    if not _VIDEO_DEBUG_ENABLED or pixel_values_videos is None:
        return
    ...
    if video_grid_thw is not None:
        grid_t = [int(v) for v in video_grid_thw[:, 0].detach().cpu().tolist()]

    temporal_patch_size = None
    vision_config = getattr(model.config, "vision_config", None)
    if vision_config is not None:
        temporal_patch_size = getattr(vision_config, "temporal_patch_size", None)

    estimated_sampled_frames = None
    if grid_t is not None and temporal_patch_size is not None:
        estimated_sampled_frames = [int(t) * int(temporal_patch_size) for t in grid_t]
...
def _get_input_embeds(
    ...
    if pixel_values is not None:
        ...
        # print('image',pixel_values.shape,image_grid_thw)
        ...
        if n_image_tokens != n_image_features:
            ...
            try:
                with open(_log_path, "a", encoding="utf-8") as f:
                    f.write(json.dumps({"time": datetime.now().isoformat(timespec="seconds"), "error": msg}, ensure_ascii=False) + "\n")
            except Exception as _e:
                print("[warn] log write failed:", _e)

            need, have = int(n_image_tokens), int(n_image_features)
            if have > need:
                image_embeds = image_embeds[:need]
                if deepstack_image_embeds is not None:
                    deepstack_image_embeds = [e[:need] for e in deepstack_image_embeds]
            elif have < need:
                pad = need - have
                z = torch.zeros((pad, image_embeds.size(-1)), dtype=image_embeds.dtype, device=image_embeds.device)
                image_embeds = torch.cat([image_embeds, z], dim=0)
                if deepstack_image_embeds is not None:
                    deepstack_image_embeds = [torch.cat([e, torch.zeros((pad, e.size(-1)), dtype=e.dtype, device=e.device)], dim=0)
                                            for e in deepstack_image_embeds]
    ...
    if pixel_values_videos is not None:
        _maybe_log_model_video_inputs(model, input_ids, pixel_values_videos, video_grid_thw)
        ...
        # print('video',pixel_values_videos.shape,video_grid_thw)


        #print('n_video_tokens', n_video_tokens)
        #print('n_video_features', n_video_features)
        # import pdb
        # pdb.set_trace()
        ...
        if n_video_tokens != n_video_features:
            ...
            need, have = int(n_video_tokens), int(n_video_features)
            if have > need:
                video_embeds = video_embeds[:need]
                if deepstack_video_embeds is not None:
                    deepstack_video_embeds = [e[:need] for e in deepstack_video_embeds]
            elif have < need:
                pad = need - have
                z = torch.zeros((pad, video_embeds.size(-1)), dtype=video_embeds.dtype, device=video_embeds.device)
                video_embeds = torch.cat([video_embeds, z], dim=0)
                if deepstack_video_embeds is not None:
                    deepstack_video_embeds = [torch.cat([e, torch.zeros((pad, e.size(-1)), dtype=e.dtype, device=e.device)], dim=0)
                                            for e in deepstack_video_embeds]
    ...
    if pixel_values is not None and pixel_values_videos is None:
        ...
        _video_grid_thw = video_grid_thw if ('video_grid_thw' in locals() and video_grid_thw is not None) \
    ...
    if pixel_values is None and pixel_values_videos is not None:
        ...
        _image_grid_thw = image_grid_thw if ('image_grid_thw' in locals() and image_grid_thw is not None) \
