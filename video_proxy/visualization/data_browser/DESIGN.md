...
将原本碎片化的 `segmentation_visualize/` 升级为一个**统一可视化系统**，直接放在 `video_proxy/visualization/data_browser/` 下，支持同一份 server + 同一个 UI 处理三类数据：
...
### Step 1：新建 `video_proxy/visualization/data_browser/server.py`
...
### Step 2：新建 `video_proxy/visualization/data_browser/index.html`
...
### Step 3：`video_proxy/visualization/data_browser/run.sh`
