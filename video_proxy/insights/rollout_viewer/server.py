...
def _extract_choice_letter(text: str) -> str:
    """Return the last standalone choice-like letter in text."""
    matches = _CHOICE_LETTER_RE.findall(text or "")
...
class RolloutStore:
    ...
    def _build_choice_meta(self, detail: dict[str, Any]) -> dict[str, Any]:
        ...
        gt_letter = _extract_choice_letter(_parse_answer_tag(gt_raw) or gt_raw)
        ...
        for attempt in detail.get("attempts", []):
            ...
            attempt["pred_letter"] = _extract_choice_letter(raw)
    ...
    def _video_file_to_frames(self, video_path: Path, max_frames: int) -> list[str]:
        # Try decord first (lightweight, more reliable). Rollout viewing is
        # intentionally time-based: one thumbnail per second, capped only to
        # keep the page responsive for long clips.
        try:
            import decord
            decord.bridge.set_bridge("native")
            vr = decord.VideoReader(str(video_path))
            ...
            if avg_fps <= 0:
                avg_fps = 1.0
            duration = total / avg_fps
            seconds = list(range(max(1, int(math.ceil(duration)))))
            if len(seconds) > max_frames:
                seconds = seconds[:max_frames]
            indices = [min(total - 1, max(0, int(round(second * avg_fps)))) for second in seconds]
            frames = []
            for idx in indices:
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path.startswith("/api/group/"):
            ...
            max_frames = max(4, min(max_frames, 200))
            timeline_fps = int(q.get("timeline_fps", ["0"])[0] or 0)
            text_only = q.get("text_only", ["0"])[0] in {"1", "true", "True", "yes"}
            try:
                detail = self.store.get_group_detail(
                    uid,
                    max_frames=max_frames,
