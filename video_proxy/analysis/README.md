...
```bash
python video_proxy/analysis/analyze_hier_seg_frame_budget.py \
  --annotation-dir /path/to/youcook2_seg/hier_seg_annotation/annotations \
  --output-dir video_proxy/analysis/outputs/hier_seg_frame_budget
```
...
```bash
bash video_proxy/analysis/run_event_logic_ablation_analysis.sh
```
...
bash video_proxy/analysis/run_event_logic_ablation_analysis.sh \
  ...
  --output-root video_proxy/analysis/outputs/my_run
```

Analyze only one experiment:

```bash
bash video_proxy/analysis/run_event_logic_ablation_analysis.sh --experiments PN
```
...
bash video_proxy/analysis/run_event_logic_ablation_analysis.sh \
  ...
  --output-root video_proxy/analysis/outputs/actual_training_problem_type_ratio
```

Fail fast if any expected input is missing:

```bash
bash video_proxy/analysis/run_event_logic_ablation_analysis.sh --strict
```
...
- Generated outputs are ignored by git via `video_proxy/analysis/.gitignore`.
