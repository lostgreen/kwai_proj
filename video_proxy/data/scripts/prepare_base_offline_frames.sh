...
BASE_FRAME_CACHE_ROOT="${BASE_FRAME_CACHE_ROOT:-${MULTI_TASK_DATA_ROOT}/offline_frames/base_cache_2fps}"

BASE_FRAME_TARGET_FPS="${BASE_FRAME_TARGET_FPS:-2.0}"
BASE_FRAME_FALLBACK_FPS="${BASE_FRAME_FALLBACK_FPS:-2.0}"
...
BASE_FRAME_UNCAPPED_EXTRACTION="${BASE_FRAME_UNCAPPED_EXTRACTION:-true}"
...
if [[ "${BASE_FRAME_UNCAPPED_EXTRACTION,,}" == "true" ]]; then
    COMMON_ARGS+=(--uncapped-extraction)
fi
...
rewrite_one() {
    ...
    python3 "${REPO_ROOT}/video_proxy/data/mixing/rewrite_videos_to_frames.py" \
...
echo "  uncapped:       ${BASE_FRAME_UNCAPPED_EXTRACTION}"
