...
#
# 流程:
#   Step 1: prepare/prepare_mcq.py — 解析 MCQ JSON → 统一 JSONL
#   Step 2: rollout/sample_pilot.py — 分层采样 pilot set (pilot 模式)
#   Step 3: offline_rollout_filter.py — rollout + reward 计算
#   Step 4: visualization/filter_and_downsample.py — 按准确率过滤 + 均匀下采样
#   Step 5: visualization/visualize.py — 分布对比图 (before vs after)
#
# 用法 (从 train/ 目录运行):
...
REWARD_FN="$SCRIPT_DIR/rollout/reward.py:compute_score"
...
if [ "$FORCE" != "1" ] && [ -s "$MCQ_JSONL" ]; then
    ...
    python "$SCRIPT_DIR/prepare/prepare_mcq.py" \
...
if [[ "$ROLLOUT_SCOPE" == "pilot" ]]; then
    ...
    if [ "$FORCE" != "1" ] && [ -s "$PILOT_JSONL" ]; then
        ...
        python "$SCRIPT_DIR/rollout/sample_pilot.py" \
            --input "$MCQ_JSONL" \
            --output "$PILOT_JSONL" \
            --per-cell "$PER_CELL" \
            --seed "$SEED"
...
if [ "$FORCE" != "1" ] && [ -s "$ROLLOUT_REPORT" ]; then
    ...
    if [ "$REPORT_COUNT" -ge "$SOURCE_COUNT" ]; then
        ...
        python "$SCRIPT_DIR/rollout/resume_helper.py" \
...
python "$SCRIPT_DIR/visualization/filter_and_downsample.py" \
    --report "$ROLLOUT_REPORT" \
    --input "$MCQ_JSONL" \
    --output "$FINAL_JSONL" \
    --min-acc "$MIN_ACC" \
...
python "$SCRIPT_DIR/visualization/visualize.py" "${VIS_ARGS[@]}"
echo ""
