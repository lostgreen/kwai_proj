...
# ── 默认参数 ────────────────────────────────────────────────────
TIMERFT_JSON="${TIMERFT_JSON:-${REPO_ROOT}/video_proxy/data/base_sources/tg/data/train_2k5.json}"
TVGBENCH_JSON="${TVGBENCH_JSON:-${REPO_ROOT}/video_proxy/data/base_sources/tg/data/tvgbench.json}"
...
process_source() {
    ...
    if [ "$SKIP_VALIDATE" = false ]; then
        ...
        python "${SCRIPT_DIR}/validate_videos.py" \
            --input "$FINAL_JSONL" \
            --output "$VALIDATED" \
            --tolerance 1.0
