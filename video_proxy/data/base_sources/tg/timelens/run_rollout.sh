...
#
# Flow:
#   Step 1: build_rollout_dataset.py — expand raw TimeLens records to query-level TG JSONL
...
BUILD_SCRIPT="${SCRIPT_DIR}/build_rollout_dataset.py"
ANALYSIS_SCRIPT="${SCRIPT_DIR}/analyze_rollout.py"
ROLLOUT_SCRIPT="${REPO_ROOT}/video_proxy/training/tools/offline_rollout_filter.py"
RESUME_HELPER="${REPO_ROOT}/video_proxy/data/base_sources/mcq/rollout/resume_helper.py"
REWARD_FN="${REPO_ROOT}/verl/reward_function/temporal_grounding_reward.py:compute_score"

INPUT_RAW="${INPUT_RAW:-${REPO_ROOT}/video_proxy/data/base_sources/tg/results/timelens_100k_short/short_pool_raw.jsonl}"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${REPO_ROOT}/video_proxy/data/base_sources/tg/results/timelens_100k_short/tg_rollout_qwen3_vl_8b_roll8}"
...
if [ "$FORCE" != "1" ] && [ -s "$QUERY_JSONL" ]; then
    ...
    echo ""
    echo "=== Step 1: build_rollout_dataset [已完成: $COUNT query items — 跳过] ==="
else
    echo ""
    echo "=== Step 1: build_rollout_dataset ==="
    BUILD_ARGS=(
        --input "$INPUT_RAW"
        --output "$QUERY_JSONL"
