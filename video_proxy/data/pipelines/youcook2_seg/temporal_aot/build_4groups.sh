...
#
# 用法:
#   bash video_proxy/data/pipelines/youcook2_seg/temporal_aot/build_4groups.sh
#
#   # 自定义参数
...
#
#   # 跳过 clip 切分 (已切好)
#   SKIP_CLIPS=true bash video_proxy/data/pipelines/youcook2_seg/temporal_aot/build_4groups.sh
# =============================================================
set -euo pipefail
set -x
...
if [[ "${SKIP_CLIPS}" != "true" ]]; then
  ...
  CMD=(
    python3 "${REPO_ROOT}/video_proxy/data/pipelines/youcook2_seg/prepare_all_clips.py"
...
_build_group() {
  ...
  python3 "${REPO_ROOT}/video_proxy/data/pipelines/youcook2_seg/temporal_aot/build_aot_from_seg.py" \
    --annotation-dir "${ANNOTATION_DIR}" \
    --clip-dir "${CLIP_ROOT}" \
    --output-dir "${out_dir}" \
