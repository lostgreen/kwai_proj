#!/usr/bin/env python3
"""
build_boundary_frame_list_data.py — 直接从原始标注 JSON 一步构建 hier-seg 训练数据。
...
用法:
    # 三层全部构建
    python build_boundary_frame_list_data.py \
...
# SCRIPT_DIR = video_proxy/data/pipelines/proxy_construction/event_boundary/
REPO_ROOT = os.path.abspath(os.path.join(SCRIPT_DIR, "..", "..", "..", "..", ".."))
ANNOTATION_DIR = os.path.join(os.path.dirname(SCRIPT_DIR), "annotation")
PROMPTS_DIR = SCRIPT_DIR
PIPELINES_DIR = os.path.join(REPO_ROOT, "video_proxy", "data", "pipelines")
ABLATION_PROMPTS_DIR = os.path.join(
    REPO_ROOT, "video_proxy", "experiments", "ablations", "hier_seg", "prompt_ablation",
)
for _p in (PROMPTS_DIR, ANNOTATION_DIR, PIPELINES_DIR, ABLATION_PROMPTS_DIR):
    if _p not in sys.path:
        sys.path.insert(0, _p)

from video_proxy.data.pipelines.proxy_construction.annotation.annotation_schema import (
    ARCHETYPE_IDS,
    TOPOLOGY_TO_DEFAULT_ARCHETYPE,
)
from video_proxy.data.pipelines.proxy_construction.event_boundary.boundary_prompts import (
    PROMPT_VARIANTS_V4,
)
from video_proxy.data.pipelines.shared.seg_source import (
    load_annotations,
...
def build_l1_records(
    ...
    视频输入: 原始视频以 l1_fps 帧率重采样后的 MP4 (由 prepare_legacy_clips.py 生成)。
    当 clip_dir_l1 为空时，videos 指向原始源视频 (可后续由 prepare_legacy_clips.py 处理)。
    """
    l1 = ann.get("level1")
