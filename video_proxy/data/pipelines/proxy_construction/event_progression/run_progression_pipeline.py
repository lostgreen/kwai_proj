#!/usr/bin/env python3
"""Orchestrate frame-list event-progression construction and rollout filtering."""
...
def _build_rollout_command(
    *,
    ...
    return [
        sys.executable,
        str((REPO_ROOT / "video_proxy" / "training" / "tools" / "offline_rollout_filter.py").resolve()),
        "--input_jsonl",
    ...
    return [
        ...
        str(
            (
                ...
                / "proxy_construction"
                / "event_progression"
                / "filter_progression_rollouts.py"
            ).resolve()
        ),
        "--report",
