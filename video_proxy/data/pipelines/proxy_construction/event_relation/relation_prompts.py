...
Usage:
    from relation_prompts import get_add_prompt, get_replace_prompt, get_sort_prompt
    from relation_prompts import get_add_t2v_prompt, get_replace_t2v_prompt
    from relation_prompts import STEP_CAPTION_SYSTEM_PROMPT, get_step_caption_prompt
"""
...
def get_replace_prompt_generic(total_steps: int, missing_pos: int, options: list[str], cot: bool = False, video_caption: str = "") -> str:
    ...
    lines += [
        ...
        "<video>",
        "",
        f"The video contains {total_steps} steps in total. "
        f"Step {missing_pos + 1} (the black screen) is the [MISSING] action.",
        "",
        "Based on what happens before and after the black screen, "
        "pick the correct textual option that describes the missing action.",
        "Options:",
    ]
    for label, opt in zip(labels, options):
        lines.append(f"{label}. {opt}")
