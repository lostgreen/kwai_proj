#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────
# run_event_relation_vlm.sh — VLM-powered event-relation data construction
#
# Frame-list pipeline:
#   Step 1: Optionally build shared source-frame cache
#   Step 2: Call LLM Task Architect to design MCQ/sort questions
#   Step 3: Assemble EasyR1 records with frame-list videos
#
# Task types:
#   1. predict_next  — Predict the next step (MCQ)
#   2. fill_blank    — Fill-in-the-blank (MCQ)
#   3. sort          — Sequence sorting (digit-sequence)
#
# Usage:
#   # Full frame-list pipeline
#   bash video_proxy/data/pipelines/proxy_construction/event_relation/run_event_relation_vlm.sh
#
#   # Test mode (5 annotations, predict_next only)
#   LIMIT=5 TASKS="predict_next" bash video_proxy/data/pipelines/proxy_construction/event_relation/run_event_relation_vlm.sh
#
#   # Legacy mp4 clip path only
#   USE_SHARED_FRAMES=0 bash video_proxy/data/pipelines/proxy_construction/event_relation/run_event_relation_vlm.sh
#
#   # Dry-run (build script texts only, no API calls, no clip cutting)
#   DRY_RUN=1 LIMIT=10 bash video_proxy/data/pipelines/proxy_construction/event_relation/run_event_relation_vlm.sh
#
# Resume: cached LLM responses are stored in $OUTPUT_DIR/cache/.
#         Re-running the same command will skip already-cached annotations.
# ─────────────────────────────────────────────────────────────────────
set -euo pipefail

export PYTHONUNBUFFERED=1

# ── Config ──────────────────────────────────────────────────────────
SCRIPT_DIR="video_proxy/data/pipelines/proxy_construction/event_relation"
CLIP_SCRIPT="video_proxy/data/pipelines/proxy_construction/legacy/clip_artifacts/prepare_all_clips.py"

# Data root (same as run_annotation_pipeline.sh)
DATA_ROOT="${DATA_ROOT:-/m2v_intern/xuboshen/zgw/data/VideoProxyMixed/proxy_annotation}"

# Derived paths
ANN_DIR="${ANN_DIR:-${DATA_ROOT}/annotations_reclassified}"
CLIP_DIR="${CLIP_DIR:-${DATA_ROOT}/clips}"
SOURCE_VIDEO_DIR="${SOURCE_VIDEO_DIR:-}"
OUTPUT_DIR="${OUTPUT_DIR:-${DATA_ROOT}/event_relation}"
SHARED_FRAME_ROOT="${SHARED_FRAME_ROOT:-${DATA_ROOT}/frame_cache/source_2fps}"
CACHE_DIR="${CACHE_DIR:-${OUTPUT_DIR}/cache}"

# LLM config
API_BASE="${API_BASE:-https://api.novita.ai/v3/openai}"
MODEL="${MODEL:-pa/gemini-3.1-pro-preview}"
TEMPERATURE="${TEMPERATURE:-0.7}"
WORKERS="${WORKERS:-8}"

# Task selection (space-separated: predict_next fill_blank sort)
TASKS="${TASKS:-predict_next fill_blank sort}"

# Data budget
TRAIN_BUDGET="${TRAIN_BUDGET:--1}"
VAL_COUNT="${VAL_COUNT:-150}"
SEED="${SEED:-42}"

# Processing limits
LIMIT="${LIMIT:-0}"
DRY_RUN="${DRY_RUN:-0}"
COMPLETE_ONLY="${COMPLETE_ONLY:-1}"
SKIP_CLIPS="${SKIP_CLIPS:-0}"
USE_SHARED_FRAMES="${USE_SHARED_FRAMES:-1}"

# Legacy mp4 clip cutting config (unused when USE_SHARED_FRAMES=1)
CLIP_LEVELS="${CLIP_LEVELS:-L2 L3}"
CLIP_FPS="${CLIP_FPS:-2}"
CLIP_WORKERS="${CLIP_WORKERS:-8}"

# Shared source-frame config (same physical cache as hier-seg / AoT)
CACHE_FPS="${CACHE_FPS:-2.0}"
FRAME_VIEW_FPS="${FRAME_VIEW_FPS:-2.0}"
BUILD_SOURCE_FRAME_CACHE="${BUILD_SOURCE_FRAME_CACHE:-0}"
SHARED_FRAME_WORKERS="${SHARED_FRAME_WORKERS:-16}"
SHARED_FRAME_JPEG_QUALITY="${SHARED_FRAME_JPEG_QUALITY:-2}"

# ── Logging ─────────────────────────────────────────────────────────
LOG_DIR="${OUTPUT_DIR}/logs"
mkdir -p "$LOG_DIR"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_FILE="${LOG_DIR}/event_relation_vlm_${TIMESTAMP}.log"

log() {
    echo "[$(date '+%H:%M:%S')] $*" | tee -a "$LOG_FILE"
}

run_step() {
    local step_name="$1"
    shift
    log "========== ${step_name} START =========="
    log "CMD: $*"
    if "$@" 2>&1 | tee -a "$LOG_FILE"; then
        log "========== ${step_name} DONE =========="
    else
        log "========== ${step_name} FAILED (exit $?) =========="
        log "Check log: $LOG_FILE"
    fi
}

# ── Print config ────────────────────────────────────────────────────
log "========== EVENT RELATION VLM PIPELINE CONFIG =========="
log "DATA_ROOT:     $DATA_ROOT"
log "ANN_DIR:       $ANN_DIR"
log "CLIP_DIR:      $CLIP_DIR"
log "OUTPUT_DIR:    $OUTPUT_DIR"
log "CACHE_DIR:     $CACHE_DIR"
log "SHARED_FRAMES: $USE_SHARED_FRAMES"
log "FRAME_ROOT:    $SHARED_FRAME_ROOT"
log "CACHE_FPS:     $CACHE_FPS"
log "VIEW_FPS:      $FRAME_VIEW_FPS"
log "MODEL:         $MODEL"
log "API_BASE:      $API_BASE"
log "WORKERS:       $WORKERS"
log "TEMPERATURE:   $TEMPERATURE"
log "TASKS:         $TASKS"
log "TRAIN_BUDGET:  $TRAIN_BUDGET"
log "VAL_COUNT:     $VAL_COUNT"
log "LIMIT:         ${LIMIT:-0 (all)}"
log "DRY_RUN:       $DRY_RUN"
log "SKIP_CLIPS:    $SKIP_CLIPS"
log "COMPLETE_ONLY: $COMPLETE_ONLY"
log "LOG_FILE:      $LOG_FILE"

# ─────────────────────────────────────────────────────────────────────
# THREE-STEP LEGACY PIPELINE (annotation-first, then targeted mp4 cutting):
#
#   Step 1: LLM designs questions (text-only) → needed_clips.txt
#   Step 2: Cut only the clips listed in needed_clips.txt
#   Step 3: Assemble final training records (--complete-only)
#
# This avoids cutting ALL L2/L3 clips upfront; the LLM first decides
# which annotations are suitable and which events to use, then we only
# cut what's actually needed.
#
# Legacy behavior (SKIP_CLIPS=1 / old two-step) is preserved via env vars.
# ─────────────────────────────────────────────────────────────────────

CLIPS_LIST="${OUTPUT_DIR}/needed_clips.txt"

# ── Common flags for build_relation_frame_list_data.py ────────────────────────
VLM_FLAGS=""
if [[ "$LIMIT" -gt 0 ]]; then
    VLM_FLAGS="$VLM_FLAGS --limit $LIMIT"
fi
if [[ "$TRAIN_BUDGET" -gt 0 ]]; then
    VLM_FLAGS="$VLM_FLAGS --train-budget $TRAIN_BUDGET"
fi

# ── Common flags for prepare_all_clips.py ────────────────────────────
CLIP_BASE_FLAGS="--complete-only --levels $CLIP_LEVELS --l2l3-fps $CLIP_FPS --workers $CLIP_WORKERS"
if [[ -n "$SOURCE_VIDEO_DIR" ]]; then
    CLIP_BASE_FLAGS="$CLIP_BASE_FLAGS --source-video-dir $SOURCE_VIDEO_DIR"
fi

# ── Shared-frame flags for build_relation_frame_list_data.py ─────────────────
FRAME_FLAGS=()
if [[ "$USE_SHARED_FRAMES" -eq 1 ]]; then
    FRAME_FLAGS=(
        --frame-cache-root "$SHARED_FRAME_ROOT"
        --cache-fps "$CACHE_FPS"
        --frame-view-fps "$FRAME_VIEW_FPS"
    )
fi

# =====================================================================
# DRY RUN: show script texts only, no API calls, no clip cutting
# =====================================================================
if [[ "$DRY_RUN" -eq 1 ]]; then
    log ""
    log ">>>>>>>>>> DRY-RUN: build script texts only <<<<<<<<<<"
    run_step "DRYRUN_SCRIPTS" \
        python "$SCRIPT_DIR/build_relation_frame_list_data.py" \
            --annotation-dir "$ANN_DIR" \
            --clip-dir "$CLIP_DIR" \
            --output-dir "$OUTPUT_DIR" \
            --api-base "$API_BASE" \
            --model "$MODEL" \
            --temperature "$TEMPERATURE" \
            --workers "$WORKERS" \
            --tasks $TASKS \
            --cache-dir "$CACHE_DIR" \
            --seed "$SEED" \
            --dry-run \
            $VLM_FLAGS

    log ""
    log "========== DRY RUN COMPLETE =========="
    log "Full log: $LOG_FILE"
    exit 0
fi

# =====================================================================
# STEP 1: LLM question design → needed_clips.txt
#   (skipped if SKIP_CLIPS=1, assuming needed_clips.txt already exists)
# =====================================================================
if [[ "$USE_SHARED_FRAMES" -eq 1 ]]; then
    log ""
    log ">>>>>>>>>> STEP 1: SKIP (USE_SHARED_FRAMES=1, using source frame cache) <<<<<<<<<<"
elif [[ "$SKIP_CLIPS" -eq 0 ]]; then
    log ""
    log ">>>>>>>>>> STEP 1: LLM QUESTION DESIGN → needed_clips.txt <<<<<<<<<<"
    run_step "S1_COLLECT_CLIPS" \
        python "$SCRIPT_DIR/build_relation_frame_list_data.py" \
            --annotation-dir "$ANN_DIR" \
            --clip-dir "$CLIP_DIR" \
            --output-dir "$OUTPUT_DIR" \
            --api-base "$API_BASE" \
            --model "$MODEL" \
            --temperature "$TEMPERATURE" \
            --workers "$WORKERS" \
            --tasks $TASKS \
            --cache-dir "$CACHE_DIR" \
            --seed "$SEED" \
            --collect-clips-only \
            $VLM_FLAGS
else
    log ""
    log ">>>>>>>>>> STEP 1: SKIP (SKIP_CLIPS=1, using existing needed_clips.txt) <<<<<<<<<<"
fi

# =====================================================================
# STEP 2: Cut only the needed clips
# =====================================================================
if [[ "$USE_SHARED_FRAMES" -eq 1 ]]; then
    log ""
    log ">>>>>>>>>> STEP 2: SKIP (USE_SHARED_FRAMES=1, no atomic mp4 cutting needed) <<<<<<<<<<"
elif [[ "$SKIP_CLIPS" -eq 0 ]]; then
    if [[ ! -f "$CLIPS_LIST" ]]; then
        log "ERROR: needed_clips.txt not found at $CLIPS_LIST — clip cutting skipped"
    else
        NEEDED_N=$(wc -l < "$CLIPS_LIST" | tr -d ' ')
        log ""
        log ">>>>>>>>>> STEP 2: CUT $NEEDED_N NEEDED CLIPS <<<<<<<<<<"
        run_step "S2_CUT_CLIPS" \
            python "$CLIP_SCRIPT" \
                --annotation-dir "$ANN_DIR" \
                --output-dir "$CLIP_DIR" \
                --clip-list "$CLIPS_LIST" \
                $CLIP_BASE_FLAGS
    fi
else
    log ""
    log ">>>>>>>>>> STEP 2: SKIP (SKIP_CLIPS=1) <<<<<<<<<<"
fi

# =====================================================================
# STEP 3: Assemble final training records (clips now exist on disk)
# =====================================================================
log ""
log ">>>>>>>>>> STEP 3: ASSEMBLE TRAINING RECORDS <<<<<<<<<<"

if [[ "$USE_SHARED_FRAMES" -eq 1 && "$BUILD_SOURCE_FRAME_CACHE" -eq 1 ]]; then
    log ""
    log ">>>>>>>>>> PREPARE SHARED SOURCE FRAME CACHE <<<<<<<<<<"
    run_step "S0_SOURCE_FRAME_CACHE" \
        python "video_proxy/data/mixing/build_source_frame_cache.py" \
            --annotation-dir "$ANN_DIR" \
            --frames-root "$SHARED_FRAME_ROOT" \
            --cache-fps "$CACHE_FPS" \
            --jpeg-quality "$SHARED_FRAME_JPEG_QUALITY" \
            --workers "$SHARED_FRAME_WORKERS"
fi

ASSEMBLE_FLAGS="--complete-only"
if [[ "$TRAIN_BUDGET" -gt 0 ]]; then
    ASSEMBLE_FLAGS="$ASSEMBLE_FLAGS --train-budget $TRAIN_BUDGET"
fi
if [[ "$LIMIT" -gt 0 ]]; then
    ASSEMBLE_FLAGS="$ASSEMBLE_FLAGS --limit $LIMIT"
fi

run_step "S3_ASSEMBLE" \
    python "$SCRIPT_DIR/build_relation_frame_list_data.py" \
        --annotation-dir "$ANN_DIR" \
        --clip-dir "$CLIP_DIR" \
        --output-dir "$OUTPUT_DIR" \
        --api-base "$API_BASE" \
        --model "$MODEL" \
        --temperature "$TEMPERATURE" \
        --workers "$WORKERS" \
        --tasks $TASKS \
        --val-count "$VAL_COUNT" \
        --cache-dir "$CACHE_DIR" \
        --seed "$SEED" \
        "${FRAME_FLAGS[@]}" \
        $ASSEMBLE_FLAGS

# ── Summary ─────────────────────────────────────────────────────────
log ""
log "========== PIPELINE COMPLETE =========="
if [[ -f "$OUTPUT_DIR/train.jsonl" ]]; then
    TRAIN_N=$(wc -l < "$OUTPUT_DIR/train.jsonl" | tr -d ' ')
    VAL_N=$(wc -l < "$OUTPUT_DIR/val.jsonl" | tr -d ' ')
    log "Output: $TRAIN_N train + $VAL_N val records"
    log "  train.jsonl: $OUTPUT_DIR/train.jsonl"
    log "  val.jsonl:   $OUTPUT_DIR/val.jsonl"
    log "  stats.json:  $OUTPUT_DIR/stats.json"
    log "  cache:       $CACHE_DIR"
    log "  needed_clips: $CLIPS_LIST"
fi

if [[ -d "$CLIP_DIR/L2" ]]; then
    L2_N=$(ls "$CLIP_DIR/L2/"*.mp4 2>/dev/null | wc -l | tr -d ' ')
    L3_N=$(ls "$CLIP_DIR/L3/"*.mp4 2>/dev/null | wc -l | tr -d ' ')
    log "Clips: $L2_N L2 + $L3_N L3"
fi

log "Full log: $LOG_FILE"
