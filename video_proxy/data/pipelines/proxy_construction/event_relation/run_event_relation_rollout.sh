#!/usr/bin/env bash
# Offline rollout filtering for event-relation MCQ tasks.
#
# Usage:
#   INPUT_JSONL=/path/to/train_predict_next.jsonl \
#   MODEL_PATH=/path/to/Qwen3-VL-8B-Instruct \
#   bash video_proxy/data/pipelines/proxy_construction/event_relation/run_event_relation_rollout.sh
#
...
if [[ -z "${INPUT_JSONL}" ]]; then
  echo "Please set INPUT_JSONL=/path/to/event_relation_train.jsonl" >&2
  exit 1
fi
...
echo "============================================="
echo " Event Relation Hard-Case Rollout"
