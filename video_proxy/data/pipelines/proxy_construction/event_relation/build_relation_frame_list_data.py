#!/usr/bin/env python3
"""
build_relation_frame_list_data.py — VLM-powered event-relation data construction pipeline.

Calls a text-only LLM "Task Architect" (via Novita/OpenAI-compatible API) to
design three types of event-relation questions from hierarchical annotations
annotations:
...
Usage:
    ...
    ANN=/m2v_intern/xuboshen/zgw/data/VideoProxyMixed/proxy_annotation/annotations
    CLIPS=/m2v_intern/xuboshen/zgw/data/VideoProxyMixed/proxy_annotation/clips
    OUT=/m2v_intern/xuboshen/zgw/data/VideoProxyMixed/event_relation/vlm

    python build_relation_frame_list_data.py \\
...
from video_proxy.data.pipelines.shared.seg_source import (  # noqa: E402
    load_annotations,
    get_l2_event_atomic_path,
    get_l3_action_atomic_path,
)
from video_proxy.data.pipelines.shared.frame_cache import (  # noqa: E402
    build_source_cache_dir,
    load_cached_frames,
    select_frame_paths_for_span,
)
from video_proxy.data.pipelines.proxy_construction.event_relation.relation_task_prompts import (  # noqa: E402
    TASK_ARCHITECT_SYSTEM_PROMPT,
    get_predict_next_user_prompt,
    get_fill_blank_user_prompt,
    get_sequence_sort_user_prompt,
)
from video_proxy.data.pipelines.proxy_construction.event_relation.relation_prompts import (  # noqa: E402
...
def build_script_text(ann: dict) -> str | None:
    ...
    L3 format: nested — grounding_results[].sub_actions[] (from build_hierarchical_annotations.py).
    """
    l1 = ann.get("level1")
    l2 = ann.get("level2")
