#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────
# run_annotation_pipeline.sh — Scene-first hierarchical annotation pipeline
#
# Steps:
...
#      + L1 phase aggregation — all within build_hierarchical_annotations.py
#
# Usage:
#   tmux new -s anno
...
#   LIMIT=5 bash video_proxy/data/pipelines/proxy_construction/annotation/run_annotation_pipeline.sh
#
...
JSONL="${JSONL:-${REPO_ROOT}/video_proxy/data/pipelines/data_curation/results/et_instruct_164k/screen_keep.jsonl}"
...
run_step "S1_EXTRACT_FRAMES" \
    python "$SCRIPT_DIR/extract_annotation_frames.py" \
        --jsonl "$JSONL" \
...
run_step "S1_5_SCENE_DETECT" \
    "$SCENE_PYTHON" "$SCRIPT_DIR/detect_scene_boundaries.py" \
...
run_step "S2_ANNOTATE" \
    python "$SCRIPT_DIR/build_hierarchical_annotations.py" \
        --jsonl "$JSONL" \
        --frames-dir "$DATA_ROOT/frames" \
