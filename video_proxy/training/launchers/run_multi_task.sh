...
REUSE_EXISTING_DATA_EFFECTIVE="${REUSE_EXISTING_DATA:-false}"
...
COT_BUDGET_REPAIR_SUFFIX="${COT_BUDGET_REPAIR_SUFFIX:-}"
...
FORMAT_LOSS_WEIGHT="${FORMAT_LOSS_WEIGHT:-}"
ANSWER_FALLBACK_AFTER_THOUGHT="${ANSWER_FALLBACK_AFTER_THOUGHT:-true}"
REWARD_KWARGS_ARGS=()
LOSS_WEIGHT_ARGS=()
if [[ -n "${FORMAT_LOSS_WEIGHT}" ]]; then
    LOSS_WEIGHT_ARGS+=(worker.reward.format_loss_weight="${FORMAT_LOSS_WEIGHT}")
fi
if [[ "${COT_FORMAT_REWARD_ENABLED}" == "true" ]]; then
    REWARD_KWARGS_ARGS+=(
        worker.reward.reward_function_kwargs="{\"cot_format_reward_enabled\":true,\"cot_format_missing\":${COT_FORMAT_REWARD_MISSING},\"cot_format_truncated\":${COT_FORMAT_REWARD_TRUNCATED},\"cot_format_ok\":${COT_FORMAT_REWARD_OK}}"
    )
fi
...
echo "[multi-task] REUSE_EXISTING_DATA=${REUSE_EXISTING_DATA_EFFECTIVE}"
...
echo "[multi-task] COT_BUDGET_ENABLED=${COT_BUDGET_ENABLED} COT_BUDGET_START_TOKEN=${COT_BUDGET_START_TOKEN} COT_BUDGET_END_TOKEN=${COT_BUDGET_END_TOKEN} COT_BUDGET_MAX_TOKENS=${COT_BUDGET_MAX_TOKENS} COT_BUDGET_REPAIR_SUFFIX=${COT_BUDGET_REPAIR_SUFFIX}"
echo "[multi-task] ACTOR_LOSS_AVG_MODE=${ACTOR_LOSS_AVG_MODE} ENABLE_RESPONSE_LOSS_WEIGHT_MASK=${ENABLE_RESPONSE_LOSS_WEIGHT_MASK} THOUGHT_LOSS_WEIGHT=${THOUGHT_LOSS_WEIGHT} ANSWER_LOSS_WEIGHT=${ANSWER_LOSS_WEIGHT} DEFAULT_LOSS_WEIGHT=${DEFAULT_LOSS_WEIGHT} FORMAT_LOSS_WEIGHT=${FORMAT_LOSS_WEIGHT} ANSWER_FALLBACK_AFTER_THOUGHT=${ANSWER_FALLBACK_AFTER_THOUGHT}"
...
elif [[ "${REUSE_EXISTING_DATA_EFFECTIVE,,}" =~ ^(true|1|yes)$ ]]; then
    NEEDS_MIX=false
    MIX_REASON="REUSE_EXISTING_DATA=${REUSE_EXISTING_DATA_EFFECTIVE}"
...
python3 -m verl.trainer.main \
    ...
    worker.rollout.cot_budget_repair_suffix="${COT_BUDGET_REPAIR_SUFFIX}" \
    ...
    worker.reward.answer_fallback_after_thought="${ANSWER_FALLBACK_AFTER_THOUGHT}" \
    "${LOSS_WEIGHT_ARGS[@]}" \
    "${REWARD_KWARGS_ARGS[@]}" \
