...
#
# 实验脚本用法:
#   source video_proxy/training/common/multi_task_common.sh
#   EXP_NAME="my_exp"
#   # (可选) REWARD_FUNCTION="..."
#   source video_proxy/training/launchers/run_multi_task.sh
# =============================================================

_COMMON_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd -- "${_COMMON_DIR}/../../.." && pwd)"
VIDEO_PROXY_ROOT="${REPO_ROOT}/video_proxy"
...
AOT_TEACHER_MODEL_PATH="${AOT_TEACHER_MODEL_PATH:-}"
AOT_TEACHER_TOKENIZER_PATH="${AOT_TEACHER_TOKENIZER_PATH:-${AOT_TEACHER_MODEL_PATH}}"
AOT_TEACHER_TRUST_REMOTE_CODE="${AOT_TEACHER_TRUST_REMOTE_CODE:-${TEACHER_TRUST_REMOTE_CODE}}"
SEG_TEACHER_MODEL_PATH="${SEG_TEACHER_MODEL_PATH:-}"
SEG_TEACHER_TOKENIZER_PATH="${SEG_TEACHER_TOKENIZER_PATH:-${SEG_TEACHER_MODEL_PATH}}"
SEG_TEACHER_TRUST_REMOTE_CODE="${SEG_TEACHER_TRUST_REMOTE_CODE:-${TEACHER_TRUST_REMOTE_CODE}}"
EVENTLOGIC_TEACHER_MODEL_PATH="${EVENTLOGIC_TEACHER_MODEL_PATH:-}"
EVENTLOGIC_TEACHER_TOKENIZER_PATH="${EVENTLOGIC_TEACHER_TOKENIZER_PATH:-${EVENTLOGIC_TEACHER_MODEL_PATH}}"
EVENTLOGIC_TEACHER_TRUST_REMOTE_CODE="${EVENTLOGIC_TEACHER_TRUST_REMOTE_CODE:-${TEACHER_TRUST_REMOTE_CODE}}"
...
TASK_HOMOGENEOUS_BATCHING="${TASK_HOMOGENEOUS_BATCHING:-false}"
TASK_HOMOGENEOUS_GROUPING="${TASK_HOMOGENEOUS_GROUPING:-raw}"
...
FILTER_KEY="${FILTER_KEY:-accuracy}"
FILTER_LOW="${FILTER_LOW:-0.05}"
FILTER_HIGH="${FILTER_HIGH:-0.95}"
COT_FORMAT_REWARD_ENABLED="${COT_FORMAT_REWARD_ENABLED:-false}"
COT_FORMAT_REWARD_MISSING="${COT_FORMAT_REWARD_MISSING:-0.0}"
COT_FORMAT_REWARD_TRUNCATED="${COT_FORMAT_REWARD_TRUNCATED:-0.5}"
COT_FORMAT_REWARD_OK="${COT_FORMAT_REWARD_OK:-1.0}"
...
OPD_DEFAULT_TEACHER="${OPD_DEFAULT_TEACHER:-}"
ACTOR_OFFLOAD_PARAMS="${ACTOR_OFFLOAD_PARAMS:-false}"
ACTOR_OFFLOAD_OPTIMIZER="${ACTOR_OFFLOAD_OPTIMIZER:-false}"
REF_OFFLOAD_PARAMS="${REF_OFFLOAD_PARAMS:-false}"
...
FILLER_BUSY_HOLD_MS="${FILLER_BUSY_HOLD_MS:-0}"
FILLER_BUSY_MATRIX="${FILLER_BUSY_MATRIX:-512}"
FILLER_BUSY_BATCH="${FILLER_BUSY_BATCH:-0}"
FILLER_BUSY_SLEEP_MS="${FILLER_BUSY_SLEEP_MS:-80}"
FILLER_IDLE_SLEEP_MS="${FILLER_IDLE_SLEEP_MS:-4}"
FILLER_ORPHAN_MATRIX="${FILLER_ORPHAN_MATRIX:-512}"
FILLER_ORPHAN_BATCH="${FILLER_ORPHAN_BATCH:-0}"
FILLER_ORPHAN_SLEEP_MS="${FILLER_ORPHAN_SLEEP_MS:-200}"
