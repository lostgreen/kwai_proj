#!/usr/bin/env bash
# Roll out a merged action/sort dataset and keep hard cases by mean_reward.
set -euo pipefail
...
TARGET_TOTAL="${TARGET_TOTAL:-10000}"
SEED="${SEED:-42}"

if [[ -z "${INPUT_JSONL}" ]]; then
  echo "Please set INPUT_JSONL=/path/to/merged_train.jsonl" >&2
  exit 1
fi
...
FILTER_SCRIPT="${SCRIPT_DIR}/filter_rollout_hard_cases.py"
...
echo "============================================="
echo " Action/Sort Hard-Case Rollout"
