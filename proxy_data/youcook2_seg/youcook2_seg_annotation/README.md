...
python proxy_data/youcook2_seg/youcook2_seg_annotation/extract_frames.py \
  ...
  --output-dir /path/to/youcook2_seg_annotation/frames \
  --fps 1.0 \
  --workers 8
```
...
```bash
# FRAMES_DIR / ANN_DIR 替换为实际路径
export FRAMES_DIR=/path/to/youcook2_seg_annotation/frames
...
# L2 审核（需要 L1+L2 已完成）
python proxy_data/youcook2_seg/youcook2_seg_annotation/annotate.py \
  --frames-dir $FRAMES_DIR --output-dir $ANN_DIR \
  --level 2c --model qwen/qwen2.5-vl-72b-instruct --workers 4

# L3 审核（需要 L2+L3 已完成）
python proxy_data/youcook2_seg/youcook2_seg_annotation/annotate.py \
  --frames-dir $FRAMES_DIR --output-dir $ANN_DIR \
  --level 3c --model qwen/qwen2.5-vl-72b-instruct --workers 4
```
...
python proxy_data/youcook2_seg/youcook2_seg_annotation/annotate_check.py \
  --frames-dir $FRAMES_DIR \
...
python proxy_data/youcook2_seg/youcook2_seg_annotation/annotate_check.py ... --dry-run

# 只做 L2 审核
python proxy_data/youcook2_seg/youcook2_seg_annotation/annotate_check.py ... --levels 2c
```
...
# 注：AoT 和 Event Logic 数据同样通过共享标注构建，详见各子目录 README
```
