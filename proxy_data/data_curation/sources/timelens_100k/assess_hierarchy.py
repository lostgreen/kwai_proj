"""
[DEPRECATED] 旧版 LLM 层次标注潜力评估脚本 — 已被两阶段 pipeline 取代

请使用新的两阶段筛选流程：
  1. stage_a_coarse_filter.py  — L2 粒度粗筛
  2. shared/stage_b_fine_filter.py — 层次潜力精筛

旧版问题：
  - 默认假设 TimeLens 标注就是 L2，未先验证粒度
  - 一步同时问粒度 + 潜力 + 结构，LLM 容易出现补偿效应
  - 缺少程序化决策规则兜底
